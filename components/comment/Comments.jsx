      ...
      >
         ...
         >
            <Space wrap style={{display: "flex", justifyContent: "space-between"}}>
               ...
               >
                  {comment?.comment?.owner?.firstName} {comment?.comment?.owner?.lastName}
               </Text>
               <Text
                  style={{
                     display: "block",
               ...
               >
                  {checkTime((new Date() - new Date(comment?.comment?.createdAt)))}
               </Text>
            </Space>
            <Text
            ...
            >
               {
                  answers?.length > 0 ?
                     <Button
                        type="link"
                        onClick={() => (answers?.filter(comment => {
                           if (showAnswers === null) {
                              return comment.userId === user?.id;
                           } else return showAnswers;
                        }).length === answers.length ?  setShow(false) : setShow(true))}
                     >
                        {
                           showAnswers ? "Hide" : (
                           ...
                           ) ? "Hide" : "Show"} {answers?.length} answers
                     </Button> : null
               }
                  ...
                  <Space direction="vertical" style={{width: "100%"}}>
                     ...
                     >
                        ...
                        >
                           ...
                           >
                              {checkTime((new Date() - new Date(answer.createdAt)))}
                           </Text>
                        </Space>
